...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const emailInput = document.getElementById('email-input'); // Get the email input
    ...
    requestCodeButton.addEventListener('click', async () => {
        ...
        if (!email) {
            requestCodeMessage.textContent = 'Please enter your email.';
            return;
        }
        ...
        try {
            ...
            if (response.ok && data.message === 'Verification code sent.') {
                ...
                startExpirationTimer(data.expiresIn * 1000 || 60000); // Use expiresIn from response if available
    ...
    verifyCodeButton.addEventListener('click', async () => {
        ...
        const email = emailInput.value; // Get the email for verification
        if (!email) {
            verifyCodeMessage.textContent = 'Email is missing.';
            return;
        }
        ...
        try {
            ...
            if (response.ok && data.message === 'Email verified.') {
                ...
                verifyCodeMessage.textContent = data.error || 'Verification failed. Please check the code.';
